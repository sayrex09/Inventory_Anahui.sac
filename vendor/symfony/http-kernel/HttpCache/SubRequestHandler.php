...
class SubRequestHandler
{
    ...
    public static function handle(HttpKernelInterface $kernel, Request $request, $type, $catch)
    {
        ...
        // fix the client IP address by setting it to localhost,
        // which is the core responsibility of this method
        $request->server->set('REMOTE_ADDR', 'localhost');

        // ensure localhost is set as trusted proxy
        if (!IpUtils::checkIp('localhost', $trustedProxies)) {
            Request::setTrustedProxies(array_merge($trustedProxies, array('localhost')), Request::getTrustedHeaderSet());
        }
