...
class FileProfilerStorageTest extends TestCase
{
    ...
    public function testStore()
    {
        for ($i = 0; $i < 10; ++$i) {
            ...
            $profile->setIp('localhost');
            $profile->setUrl('http://foo.bar');
            $profile->setMethod('GET');
            $this->storage->write($profile);
        }
        $this->assertCount(10, $this->storage->find('localhost', 'http://foo.bar', 20, 'GET'), '->write() stores data in the storage');
    }

    public function testChildren()
    {
        $parentProfile = new Profile('token_parent');
        $parentProfile->setIp('localhost');
        $parentProfile->setUrl('http://foo.bar/parent');

        $childProfile = new Profile('token_child');
        $childProfile->setIp('localhost');
    ...
    public function testRetrieveByIp()
    {
        ...
        $this->assertCount(1, $this->storage->find('localhost', '', 10, 'GET'), '->find() retrieve a record by IP');
    ...
    public function testRetrieveByUrl()
    {
        ...
        $this->assertCount(1, $this->storage->find('localhost', 'http://foo.bar/\'', 10, 'GET'), '->find() accepts single quotes in URLs');
        $this->assertCount(1, $this->storage->find('localhost', 'http://foo.bar/"', 10, 'GET'), '->find() accepts double quotes in URLs');
        $this->assertCount(1, $this->storage->find('localhost', 'http://foo\\bar/', 10, 'GET'), '->find() accepts backslash in URLs');
        $this->assertCount(1, $this->storage->find('localhost', 'http://foo.bar/;', 10, 'GET'), '->find() accepts semicolon in URLs');
        $this->assertCount(1, $this->storage->find('localhost', 'http://foo.bar/%', 10, 'GET'), '->find() does not interpret a "%" as a wildcard in the URL');
        $this->assertCount(1, $this->storage->find('localhost', 'http://foo.bar/_', 10, 'GET'), '->find() does not interpret a "_" as a wildcard in the URL');
    }
    ...
    public function testPurge()
    {
        ...
        $this->assertCount(1, $this->storage->find('localhost', '', 10, 'GET'));
        ...
        $this->assertCount(2, $this->storage->find('localhost', '', 10, 'GET'));

        $this->storage->purge();

        $this->assertEmpty($this->storage->read('token'), '->purge() removes all data stored by profiler');
        $this->assertCount(0, $this->storage->find('localhost', '', 10, 'GET'), '->purge() removes all items from index');
    }
    ...
    public function testDuplicates()
    {
        ...
        $this->assertCount(3, $this->storage->find('localhost', 'http://example.net/', 3, 'GET'), '->find() method returns incorrect number of entries');
    }
